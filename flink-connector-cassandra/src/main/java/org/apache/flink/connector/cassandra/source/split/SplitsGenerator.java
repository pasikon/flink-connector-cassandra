...
public final class SplitsGenerator {
    ...
    private float getRingFraction(List<TokenRange> tokenRanges) {
        ...
        float newRingFraction =
                new BigDecimal(addressedTokens)
                        .divide(new BigDecimal(partitioner.ringSize), 6, RoundingMode.HALF_UP)
                        .floatValue();
        LOG.info(
                "OLD ring fraction: {}", addressedTokens.divide(partitioner.ringSize).floatValue());
        LOG.info("NEW ring fraction: {}", newRingFraction);
        return newRingFraction;
    }
